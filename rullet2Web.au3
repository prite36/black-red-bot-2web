...
HotKeySet("{INSERT}","_goToRoulette_Dafabet")
...
Global  $profitPerRound =  Random(10,20,1)  ; profit 10 - 20 bath per round
...
Func Start_Dafabet()
	Global $Title = "Dafabet"
	Global $filePath = @ScriptDir&"\LogFile\BetMoney_Dafabet.txt"
...
Func Start_W88()
	Global $Title = "W88"
	Global 	$filePath = @ScriptDir&"\LogFile\BetMoney_W88.txt"
	ToolTip('InternetCheck ', 19,0 )
	InternetCheck ()
	ToolTip('Bot Start ', 19,0 )
	$timeRestart = TimerInit()  ;เริ่มจับเวลา
	While 1
		Local $hTimer = TimerInit()  ;เริ่มจับเวลา
		While 1
			ToolTip('Loop while 2 '&Floor(TimerDiff($hTimer)/1000), 19,0 )
			if  search_area("w88CountdownTime") And search_area("w88FillMoney")  Then  ; ถ้าตัวจับเวลาเปลี่ยนเป็นสีดำ และ ช่องใส่เงินมีสีขึ้น
				ToolTip('New Round ', 50,0 )
				local $moneyOnFile  = FileReadLine($filePath,1)  ;เปิดไฟล์ดึงค่า
				If search_area("w88MenuNow10-200THB") and $moneyOnFile > 200 Then
					click_picture("w88MenuRoulette")
					click_picture("w88Menu100-2000THB")
					sleep(1500)
				ElseIf search_area("w88MenuNow100-2000THB") and $moneyOnFile < 200 Then
					click_picture("w88MenuRoulette")
					click_picture("w88Menu10-200THB")
					sleep(1500)
				EndIf
				sleep(500)
				click_picture("w88FillMoney")
				Sleep(300)
				send(Floor($moneyOnFile)) ;ใส่เงินในช่อง
				$Money = $moneyOnFile  ;แทนค่าในตัวแปร เพื่อเอาไปเช็ค
				FileClose($filePath) ;ปิด File
				...
				Sleep(300)
					if $dropMoney == 1 then
						click_picture("w88AreaLeft")
					ElseIf $dropMoney == 2 then
						click_picture("w88AreaMiddle")
					ElseIf $dropMoney == 3 then
						click_picture("w88AreaRight")
					EndIf
				click_picture("w88SubmitButton") ;คลิกปุ่ม ยืนยัน
				ExitLoop
			EndIf
		...
		if  $winStatus == true Then
			...
			if $Money > 583   Then
				_FileWriteLog(@ScriptDir & "\LogFile\LogData_W88.log", "W88 Warning")  ;เก็บสถานะเ ลงในไฟล์ .log
				goWriteFile()
				warning ()
			EndIf
			...
			If $profitNow >= $profitPerRound then
				...
				_FileWriteLog(@ScriptDir & "\LogFile\LogData_W88.log", "Profit in day "&$profitNow&" Bath")
				goWriteFile("TotalProfit")
				$Money = 10.1
				goWriteFile()
...
Func warning ()
	...
	while 1
		...
		Sleep (1000)
		if $delayTime <= 0 Then
			if $Title == "Dafabet" Then  _goToRoulette_Dafabet()
			if $Title == "W88" Then  _goToRoulette_W88()
		EndIf
		$delayTime-=1
	WEnd
EndFunc
...
Func goWriteFile($menu="BetMoney")
	if $Title == "Dafabet" Then Global 	$filePath = @ScriptDir&"\LogFile\BetMoney_Dafabet.txt"
	if $Title == "W88" 	   Then Global 	$filePath = @ScriptDir&"\LogFile\BetMoney_W88.txt"
	if $menu == "BetMoney" Then
		Local $fileOpen = FileOpen($filePath,2)
		FileWrite($fileOpen, $Money)
		FileClose($filePath) ;ปิด File
	ElseIf $menu =="TotalProfit" Then
		Local $filePathTotalProfit = @ScriptDir&"\LogFile\TotalProfit.txt"
		local $time  = FileReadLine($filePathTotalProfit,1)  ;เปิดไฟล์ดึงค่า
